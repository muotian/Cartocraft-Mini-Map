execute if entity @a[distance=..3] run function thecookieorange:mob/phantom_wind/phantom_died

particle minecraft:cloud ~ ~ ~ 0.4 0.4 0.4 0.02 2 force

scoreboard players add @s orange_debug 1

kill @s[scores={orange_debug=2000}] 

effect give @s[team=orange_wind_charge_zombie] glowing 1 1 true

execute as @s[team=!orange_wind_charge_zombie] run team join orange_wind_charge_zombie
