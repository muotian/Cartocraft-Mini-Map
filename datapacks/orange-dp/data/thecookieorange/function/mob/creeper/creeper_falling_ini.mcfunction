...
execute if entity @a[distance=..6] run effect give @a[distance=..6] minecraft:slowness 2 0 true

execute as @a[distance=..4] run execute at @s run damage @s 1 minecraft:freeze
...
# [Distance: 4-7]
execute if score @s orange_timer matches 80..130 at @s as @e[distance=3..7,type=!#minecraft:hostile, type=!marker] at @s facing entity @e[tag=current_center,limit=1,distance=4..7] feet run function thecookieorange:mob/creeper/pull/pull_player_weak
