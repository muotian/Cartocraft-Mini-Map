advancement revoke @s only red_wool:items/totem_of_peace
execute if score @s red_wool.totem_of_peace matches ..0 run return 0
execute store result score tnt_axe red_wool.tmp run clear @p gunpowder 0
execute if score tnt_axe red_wool.tmp matches ..15 run return fail

clear @s gunpowder 16
