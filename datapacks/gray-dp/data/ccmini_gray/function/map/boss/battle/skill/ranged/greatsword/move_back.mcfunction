execute facing entity @e[type=parched,tag=gray.boss.hitbox,limit=1] feet run tp @s ^ ^ ^0.8
...
execute positioned ~-0.5 ~-0.5 ~-0.5 if entity @e[dx=0,dy=0,dz=0,type=parched,tag=gray.boss.hitbox,limit=1] run scoreboard players set $skill_timer gray.boss.data 3